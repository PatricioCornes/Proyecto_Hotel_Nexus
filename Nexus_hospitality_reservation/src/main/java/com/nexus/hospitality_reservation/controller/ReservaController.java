...
public class ReservaController {
    ...
    @GetMapping("/test-habitacion/{id}")
    public ResponseEntity<?> probarConexionConHabitaciones(@PathVariable Integer id) {
        try {
            // Aquí el microservicio de Reservas hace la llamada REST al de Habitaciones
            HabitacionDTO habitacion = habitacionClient.obtenerHabitacion(id);
            
            // Si funciona, devolvemos un mensaje de éxito junto con los datos recibidos
            return ResponseEntity.ok(Map.of(
                "mensaje", "¡Conexión exitosa!",
                "datosDeHabitacion", habitacion
            ));
        } catch (Exception e) {
            // Si falla (ej. el otro microservicio está apagado), capturamos el error
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body("Error al conectar con el microservicio de habitaciones: " + e.getMessage());
        }
    }
